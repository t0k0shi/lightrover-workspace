...
def build_velocity_log(
    client: InfluxDBClient, bucket: str, org: str, start: str, stop: str, robot_filter: str, query_params: dict
) -> pd.DataFrame:
    ...
    odom_df = query_api.query_data_frame(
        _flux_velocity_log(bucket, start, stop, robot_filter), org=org, params=query_params
    )
    ...
    cmd_df = query_api.query_data_frame(
        _flux_cmd_vel_log(bucket, start, stop, robot_filter), org=org, params=query_params
    )
    ...
    col_order = ["timestamp", "robot_id", "cmd_linear_x", "odom_linear_x", "cmd_angular_z", "odom_angular_z", "speed"]
...
def build_trajectory(
    ...
    query_api = client.query_api()
    df = query_api.query_data_frame(_flux_trajectory(bucket, start, stop, robot_filter), org=org, params=query_params)
...
def build_daily_summary(
    ...
    query_api = client.query_api()
    df = query_api.query_data_frame(_flux_daily_stats(bucket, start, stop, robot_filter), org=org, params=query_params)
    if isinstance(df, list):
        df = pd.concat(df, ignore_index=True) if df else pd.DataFrame()
    if df.empty:
        return pd.DataFrame()
    ...
    col_order = ["date", "robot_id", "session_count", "total_distance_m", "avg_speed_ms", "max_speed_ms"]
...
def parse_args() -> argparse.Namespace:
    """コマンドライン引数をパースする。"""
    parser = argparse.ArgumentParser(description="InfluxDB の走行データを Power BI 用 CSV/Parquet にエクスポート")
    parser.add_argument("--start", type=str, default=None, help="開始日 ISO8601（デフォルト: 7日前）")
    parser.add_argument("--end", type=str, default=None, help="終了日 ISO8601（デフォルト: 今日）")
    parser.add_argument("--robot-id", type=str, default=None, help="フィルタするrobot_id（省略で全ロボット）")
    parser.add_argument(
        "--format", type=str, choices=["csv", "parquet"], default="csv", help="出力フォーマット（デフォルト: csv）"
    )
    parser.add_argument("--output", type=str, default="./export", help="出力ディレクトリ（デフォルト: ./export）")
    parser.add_argument("--url", type=str, default="http://localhost:8086", help="InfluxDB URL")
    parser.add_argument("--token", type=str, default=None, help="InfluxDB トークン（デフォルト: $INFLUXDB_TOKEN）")
    parser.add_argument("--org", type=str, default="lightrover", help="InfluxDB 組織名（デフォルト: lightrover）")
    parser.add_argument("--bucket", type=str, default="seminar", help="InfluxDB バケット名（デフォルト: seminar）")
...
def main() -> None:
    ...
    try:
        ...
        trajectory = build_trajectory(client, args.bucket, args.org, start_str, stop_str, robot_filter, query_params)
