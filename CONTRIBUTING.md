...
コードの変更は TDD サイクルで進めてください:

```
① Red   : 失敗するテストを先に書く
② Green : テストが通る最小限の実装をする
③ Refactor: 動いた状態を保ちながら整理する
```

変更後、ローカルで lint とテストを通してください:
...
**注意:** ドキュメントのみの変更（`docs/`, `*.md`）はテスト不要です。
...
| 3 | 新コード = 新テスト（TDD） | コード追加時は必ず対応するテストを追加する。Red → Green → Refactor サイクルを守ること |
